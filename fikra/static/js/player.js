...
document.addEventListener("DOMContentLoaded", function() {
	...
	var explotPlayer = [];
	_VPD.player = explotPlayer;
	var plId = 0;
	var canvasDomIdPrefix = 'main-canvas';
	var canvasDomId = canvasDomIdPrefix + plId;
	var xmlLocaleDirName = 'xmllocale';
	var durationCoef = 1;
	if (typeof _VPD.durationCoef !== 'undefined') {
		durationCoef = parseFloat(_VPD.durationCoef);
	}
	var authorMode = false;
	if (typeof _VPD.authorMode !== 'undefined') {
		authorMode = _VPD.authorMode;
	}
	var varsPresetJson = null;
	if (typeof _VPD.varsPreset !== 'undefined') {
		varsPresetJson = _VPD.varsPreset;
	}
	console.log(varsPresetJson);
	...
	var ctrTline = document.getElementById('controlbar-timeline');
	...
	var processAndPlay = function (overrides) {
		...
		explotPlayer[plId].authoringMode = authorMode;
